...
GameObj::GameObj()
    : _currentScene{std::make_unique<GameScene_MainMenu>(*this)}
{}
...
void GameObj::Start()
{
    ...
    while (running)
    {
        running = _currentScene->Update();
        _display.RenderMainMenu(); // Move render to scene
    }
...
void GameObj::ChangeScene(uint8_t newGameSceneID)
{
    switch (newGameSceneID)
    {
    case GAMESCENE_ID_MAINMENU:
    {
        // TODO, use memory pooling to save Heap allocations?
        _currentScene.reset(new GameScene_MainMenu(*this));
        break;
    }
    case GAMESCENE_ID_LOCALPLAY:
    {
        // TODO, use memory pooling to save Heap allocations?
        _currentScene.reset(new GameScene_LocalPlay(*this));
        break;
    }
    default:
    {
        printf("newGameSceneID not found!\n");
    }
        break;
    }
}
